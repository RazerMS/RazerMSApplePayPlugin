//
//  APManager.swift
//  RazerMSApplePayPlugin
//
//  Created by Abd Qayyum on 24/08/2022.
...
public class APManager: NSObject {
    ...
    @objc
    public static let shared = APManager()
    
    @objc
    public var paymentData: NSMutableDictionary = [
    ...
    @objc
    public var resultPayment = [String:Any]()
...
extension APManager: PKPaymentAuthorizationControllerDelegate {
    ...
    public func paymentAuthorizationController(_ controller: PKPaymentAuthorizationController, didAuthorizePayment payment: PKPayment, handler completion: @escaping (PKPaymentAuthorizationResult) -> Void) {
        ...
        APVKey = paymentData["verifyKey"] as! String
        APRMSSignature = MD5(string: "\(APTxnAmount)\(APMerchantID)\(APReferenceNo)\(APVKey)")
            ...
            if response.statusCode == 200 {
                ...
                if let json = content as? [String: Any] {
                    ...
                    if APStatus == 0 {
                        ...
                        self.resultPayment = json
                        
                        status = .failure
                    } else {
                        let txnData = json["TxnData"] as! [String: Any]
                        var requestData = txnData["RequestData"] as! [String: Any]
                        ...
                        requestData.removeValue(forKey: "xdkHTMLRedirection")
                        self.resultPayment = requestData
                    }
                }
            } else {
                status = .failure
   ...
   public func paymentAuthorizationControllerDidFinish(_ controller: PKPaymentAuthorizationController) {
       ...
       controller.dismiss {
           DispatchQueue.main.async  {
               if self.isStatusData == "00" || self.isStatusData == "11" {
